...
![项目预览图1](./assets/preview-1.png)

![项目预览图2](./assets/preview-2.png)

![项目预览图3](./assets/preview-3.png)
...
首个稳定版本。核心变化是**正式对接上游统一数据格式**，并完成工程化加固：

- **重构：**扫描脚本 `scan-skins.js` 翻译层重写，适配上游 `sgs_bwiki_heros` / `sgs_bwiki_skins` 统一后的数据格式（全中文字段 + `{ meta, data }` 信封 + 以「武将名」为跨管线对齐锚点），数据管线端到端打通，跨管线互补与迁移成本大幅降低
- **新增：**vitest 单元测试框架，覆盖扫描脚本字段解析与详情页版本切换逻辑，保障后续改动不破坏数据正确性
- **修复：**武将技能版本切换（经典/界限突破/国战）键名未对齐，导致详情页误显示「该版本暂无技能数据」
- **优化：**卡片卡包标签显示完整卡包名（如「限定-隐山之玉」），不再仅显示大类
- **修复：**语音播放互斥，点击新语音自动停止旧语音，避免叠加；顺带移除未被任何组件引用的孤儿组件 `VoicePlayer`
- **新增：**皮肤详情页键盘操作——`↑` / `↓` 在**同一武将的不同皮肤**间切换（绝不跳到别的武将），`←` / `→` 切换**同一皮肤的不同形式**（大图 / 静态 / 动态 GIF / 动态登场）；全屏看图时不响应方向键，先按 Esc 退出大图。界面保留「当前/总数」角标与操作提示文案，未放置屏幕箭头按钮

### v0.3.1

- **修复：**搜索栏输入中文拼音时页面提前刷新，未等选字落下就触发筛选。现监听输入法组合事件（compositionstart/end），拼音组字期间只更新输入框、不触发筛选，选字确认后才发起搜索，消除输入过程中的画面抖动
